...
| DP-17 | Atendimentos CVCRM (`gold.fato_atendimentos_cvcrm`) | Protocolo/ticket de pós-venda (assistência técnica, financeiro, contratos...), 1 linha por atendimento | **API CVDW** — endpoint novo `atendimentos`, fora dos 19+1 objetos já descobertos | Bronze (`bronze.atendimentos_cvcrm`, novo objeto) + Silver + Gold (concluído) | Sondado ao vivo em 16/set/2026 (mesmo método do DP-15: `path` não documentado em lugar nenhum do repo, só confirmado testando contra a API de verdade). Só `atendimentos` respondeu; os sub-recursos plausíveis (`atendimentos/historico`, `/mensagens`, `/tipos`, `/situacoes`) devolveram todos HTTP 405, então não existem — é um endpoint plano, sem drill-down. **Sufixo `_cvcrm` em bronze/silver/gold de propósito**: o projeto já tinha `gold.fato_atendimentos` e a página `powerbi/PAGINA_ATENDIMENTO.md`, mas são do **Blip** (chat de WhatsApp) — domínio completamente diferente (protocolo formal de pós-venda vs. conversa de chat). Decisão confirmada com o desenvolvedor antes de nomear qualquer objeto, pra não colidir com o que já está em produção. **Achado de volumetria:** o ambiente `pafil` tem hoje só **10 atendimentos no total** (população inteira, não amostra — conferido via `total_de_registros` sem filtro de data), contra 4,7 mil reservas ou 58 mil leads — o módulo de protocolo do CVCRM parece recém-adotado ou pouco usado na prática; o volume real só vai aparecer com o tempo. **Tipos da bronze:** inferidos programaticamente sobre a população inteira (reaproveitando `cvdw/tipos.py`, os mesmos geradores automáticos citados no DP-15 não existem mais no repo), com 6 correções manuais por precedente de campo homônimo em tabelas irmãs: `documento_cliente`/`protocolo`/`unidade`/`cep_cliente` forçados para `text` (identificadores, perderiam zero à esquerda como `numeric`; a amostra de 10 não tinha nenhum caso com zero à esquerda, mas é questão de tempo), e `idcorretor`/`idimobiliaria` forçados para `numeric` (vieram 0/10 preenchidos, então a inferência automática cairia para `text`; `numeric` é o tipo das colunas homônimas em `bronze.corretores`/`bronze.imobiliarias`, necessário pra eventual join futuro). **Sem join com `gold.dim_corretor` por ora**: `idcorretor`/`corretor` vêm vazios nos 10 registros existentes — a mesma cascata de `chave_nome()` usada em `fato_leads`/`fato_precadastros` entra quando o campo passar a vir preenchido. **Validado sem tocar produção**: como o Postgres local está morto (ver memória `local-postgres-userspace`) e o túnel de produção não estava aberto nesta sessão, a DDL + silver + gold foram aplicadas de ponta a ponta contra um cluster descartável (`initdb`/`pg_ctl` na porta 5439, mesma receita usada na validação do DP-16), com a ingestão REAL (`ingestao.py --objetos atendimentos_cvcrm --full`) trazendo os 10 registros ao vivo da API — confirma que schema, tipos e joins (`gold.dim_empreendimento`, `empreendimento_conformado`) funcionam ponta a ponta. Falta rodar a ingestão de verdade contra produção (`python ingestao.py --full --objetos atendimentos_cvcrm --criar-tabelas` pelo túnel) e `aplicar_tudo.py` pra materializar lá. Página do Power BI: `powerbi/PAGINA_ATENDIMENTO_CVCRM.md` |
...
| R25 | Crítico, concluído: `DEPARA_LEADS_XLSM` apontava para um caminho que nunca existiu, deixando canal/mídia em branco para a maioria dos leads recentes, sem nenhum erro visível | `popular_seeds.py`, `.env`, `.env.example`, `silver.dpara_canal_midia_dc`, `gold.fato_leads` | Achado em 25/ago/2026, ao validar os dashboards de Leads em produção: o donut "Canal x Prospects" mostrava 56% "(Em branco)", e "Mídia x Prospects" tinha "(Em branco)" como a maior barra, disparado. Causa raiz: o `.env` (local e, por herança, o de produção) tinha `DEPARA_LEADS_XLSM` apontando para `BI V3 CVDW\depara\Base de Leads.xlsm` — um caminho que **nunca existiu**. `popular_seeds.py` já trata isso com tolerância (`carregar_leads_apoio` só roda se o arquivo existir; senão, um `WARNING` no log e os de-paras de lead "ficam como estão") — o problema é que "como estão" nunca tinha sido populado por essa via: a única fonte viva de `dpara_canal_midia_dc` (o de-para que decide canal/mídia para leads criados **depois de 31/out/2024**, ou seja, praticamente todo o volume recente) é justamente a aba Apoio desse arquivo; sem ela, a tabela ficava vazia e todo lead recente saía sem match. O caminho certo é a pasta `Leads\Base de Leads.xlsm` (a fonte canônica documentada em `config/deparas.yml`) — as 4 cópias em `BI V3 CVDW\depara\depara_*\arquivo\Base de Leads.xlsm` são materializações geradas por `montar_estrutura_depara.py`, não a origem, e nenhuma delas bate com o caminho que estava configurado. **Corrigido**: `.env` (local, e replicar em produção) e o exemplo em `.env.example`. **Resultado, validado localmente**: `dpara_canal_midia_dc` foi de vazio/quase vazio para 1.632 linhas, e o vazio em `gold.fato_leads` caiu de 56% para **0,2%** (115 de 58.737 leads — resíduo real, não sistêmico). Isso também deve ter inflado artificialmente qualquer KPI de "Vendas Digitais"/"House" que dependa de `canal 2.0 = "Lead"` (KPI-22, KPI-30) por meses, já que lead sem canal classificado não entra nesse filtro — vale reconferir esses números depois da correção em produção |
