...
| R19 | ⚠️ `base_precos.xlsm` tem qualidade de dado inconsistente entre abas (cada produto foi montado por cópia manual da matriz-modelo e divergiu): (1) aba QBV2 (Quinta da Boa Vista) tinha cabeçalhos com erro de digitação ("Unidde"/"ID_Prço"/"Áre Privtiv"/"Permut") que zeravam a coluna Unidade pro produto inteiro — **corrigido** via alias de coluna no loader + preferindo a tabela duplicada boa da aba QBV ("Matriz_F162427", nome mangulado por copiar/colar do Excel, mas cabeçalho correto); (2) aba VPQ (Villas do Parque - Casas e Lotes Mistos) tem `Bloco`/`Unidade` = literalmente o texto `"#N/A"` (fórmula quebrada na origem) — **NÃO dá pra corrigir por normalização**, zera VSO pros ~800 unidades desses 2 produtos (0% mesmo eles tendo vendas reais) até alguém consertar a fórmula na planilha | `silver.d_estrutura`, `popular_seeds.py` | Avisar o dono da planilha sobre o `#N/A` nas abas VPQ; enquanto isso, VSO/Estoque de "Villas do Pq." não é confiável |
